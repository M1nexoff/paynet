...
class TransferVerifyScreen(private val phone: String) : Screen {
    @Composable
    override fun Content() {
        val viewModel: TransferVerifyContract.Model = hiltScreenModel()
        val state = viewModel.collectAsState()
        when(state.value) {
            TransferVerifyContract.UiState.Success -> {
                Box(modifier = Modifier
                    ...
                    Column(
                        ...
                        Image(painter = painterResource(id = R.drawable.success), modifier = Modifier.size(100.dp), contentDescription = "")
                        Text(
                    ...
                    AppButton(
                        ...
                        viewModel.onEventDispatcher(TransferVerifyContract.Intent.Back)
                    }
                }
...
fun VerifyScreenContent(
    state: State<TransferVerifyContract.UiState>?,
    onEventDispatcher: (TransferVerifyContract.Intent) -> Unit,
    phone: String,
) {
    val remainingTime = remember { mutableIntStateOf(0) }

    when (state?.value) {
        is TransferVerifyContract.UiState.Time -> {
            remainingTime.value = (state.value as TransferVerifyContract.UiState.Time).time
        }
        TransferVerifyContract.UiState.Default -> {
    ...
    Column(
        ...
        Row(
            ...
            IconButton(onClick = {
                onEventDispatcher.invoke(TransferVerifyContract.Intent.Back)
            }) {
                Icon(
                ...
                TextField(
                    ...
                    onValueChange = { newValue ->
                        ...
                        if (otpValues.value.all { it.isNotEmpty() }){
                            ...
                            onEventDispatcher.invoke(TransferVerifyContract.Intent.TransferVerify(code))
                        }
                    },
        ...
        Button(
            onClick = {
                onEventDispatcher.invoke(TransferVerifyContract.Intent.TransferResend)
            },
            colors = ButtonDefaults.buttonColors(containerColor = if (remainingTime.intValue > 0) LightGray else Color.Blue),
            shape = RoundedCornerShape(50),
            modifier = Modifier
                .fillMaxWidth()
                .height(56.dp),
            enabled = remainingTime.intValue == 0
        ) {
            Row(
                verticalAlignment = Alignment.CenterVertically,
            ) {
...
@Preview
@Composable
fun PreviewTransferVerifyScreen() {
    Box(modifier = Modifier
        .fillMaxSize(1f)
        .background(Color.White)) {
        Column(
